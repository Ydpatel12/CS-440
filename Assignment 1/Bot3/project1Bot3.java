...
public class project1Bot3 {
    ...
    private static final char BOT_LOCATION = '3';
    ...
    public static char[][] generateShip(int size) {
        ...
        // Open the first square at random
    ...
    public static boolean notBlocked(char[][] grid, int size, int x, int y) {
        return x >= 0 && x < size && y >= 0 && y < size && grid[x][y] != BLOCKED_CELL;
    }

    public static boolean hasAdjacentFire(char[][] grid, int size, int x, int y) {
        int[] dx = {0, 0, -1, 1};
        int[] dy = {-1, 1, 0, 0};
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (newX >= 0 && newX < size && newY >= 0 && newY < size && grid[newX][newY] == FIRE_CELL) {
                return true;
            }
        }
    ...
    public static ArrayList<int[]> BFSBOT3(char[][] array, int size, int[] bot, int[] button) {
        ...
        boolean avoidAdjacentFire = false;
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (notBlocked(array, size, newX, newY) && !visited[newX][newY]) {
                    ...
                    if (avoidAdjacentFire && hasAdjacentFire(array, size, newX, newY)) {
                        continue;
                    }
            ...
            //This part is for finding cells that are next to fire cells
            //Last resort
            if (queue.isEmpty() && !avoidAdjacentFire) {
                queue.clear();
                for (int i = 0; i < size; i++) {
    ...
    public static void spreadFire(char[][] grid, int size, double flammability) {
        char[][] newGrid = new char[size][size]; // Create a new grid to avoid updating cells immediately
    ...
    public static void main(String[] args) {
        ...
        for (int timeStep = 0; timeStep < size * size; timeStep++) {
            if (timeStep == 0) {
                ...
                ArrayList<int[]> bot3Path = BFSBOT3(grid, size, botLoc, buttonLoc);
                if (bot3Path != null && !bot3Path.isEmpty()) {
                    int[] nextMove = bot3Path.remove(0);
